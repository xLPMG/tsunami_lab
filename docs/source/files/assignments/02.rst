...
After inserting the numbers we get :math:`h = 8.75` m and :math:`u = \frac{1}{15} \frac{m}{s}` and use them to compute the wavespeed
...
We are only interested in the wavespeed going to the right, thus we only compute :math:`\lambda_{2}`.
...
To attain the time for evacuating the village, we only have to devide the distance by the wavespeed.
As a finally result, in our scenario, we have 44 minutes before the wave reaches the village.
